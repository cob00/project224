...
// ============================================================================
// PCR Init: builds a[i] = -L[i,i-1]/L[i,i] and c[i,col] = rhs[i,col]/L[i,i]
// Uses ONLY L[i,i-1] and L[i,i]; ignores all other entries (handled by refinement)
// ============================================================================
__global__ void pcr_init_full(
    unsigned int numRows,
    unsigned int* rowPtrs, unsigned int* colIdxs, float* matValues,
    float* rhs, unsigned int numCols,
    float* a_coeff, float* c_coeff
) {
...
// ============================================================================
// PCR Step: doubles the dependency reach
// ============================================================================
...
// ============================================================================
// PCR Finalize: write final solution
// ============================================================================
__global__ void pcr_finalize(
    unsigned int numRows, unsigned int numCols,
    float* c_coeff, float* xValues
) {
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= (int)numRows) return;
    for (unsigned int col = 0; col < numCols; col++) {
...
// ============================================================================
// Compute residual r = B - L*X using FULL matrix L
// ============================================================================
__global__ void compute_residual(
    ...
    for (unsigned int col = 0; col < numCols; col++) {
        ...
        for (unsigned int j = rowPtrs[row]; j < rowPtrs[row + 1]; ++j) {
            unsigned int c = colIdxs[j];
            sum -= matValues[j] * X[c * numCols + col];
        }
...
__global__ void update_solution(
    float* X, float* dX, unsigned int numRows, unsigned int numCols
) {
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= (int)numRows) return;
    for (unsigned int col = 0; col < numCols; col++) {
...
// ============================================================================
// Fallback kernel (kernel 2 logic) for non-tridiagonal-like matrices
// ============================================================================
...
// ============================================================================
// Host: Detect tridiagonal-like ratio, dispatch to PCR+refinement or fallback
// ============================================================================
                 ...
                 CSCMatrix* L_c_host, CSRMatrix* L_r_host, unsigned int numCols)
{
    ...
    // Count fraction of rows that are tridiagonal-like
    int strict_count = 0;
    for (unsigned int i = 0; i < n; ++i) {
        bool is_tridiag = true;
        for (unsigned int j = L_r_host->rowPtrs[i]; j < L_r_host->rowPtrs[i + 1]; ++j) {
            unsigned int c = L_r_host->colIdxs[j];
            if (c < i && c != i - 1) { is_tridiag = false; break; }
        }
        if (is_tridiag) strict_count++;
    }
    float ratio = (n > 0) ? (float)strict_count / n : 0.0f;
    ...
    if (ratio > 0.5f) {
        printf("  [PCR + iterative refinement, tridiag ratio=%.2f%%]\n", ratio * 100);

        size_t a_size = n * sizeof(float);
        size_t c_size = n * numCols * sizeof(float);
        ...
        for (int iter = 0; iter < MAX_REFINE_ITERS; iter++) {
            // Compute residual r = B - L*X using full L
            compute_residual<<<grid, block>>>(n, csr_shadow.rowPtrs, csr_shadow.colIdxs,
                                              csr_shadow.values, x_shadow.values, b_shadow.values,
                                              residual, numCols);
        ...
        printf("  [Fallback mode, tridiag ratio=%.2f%%]\n", ratio * 100);
